...
const StarShip = ({ data }) => {
	...
	useEffect(() => {
		...
		urlPilots.forEach((element, index) => {
			setIsLoading(true);
			fetchData(element, index).catch((error) => console.error(error));
		});
	}, []);
	// fi pilots
				...
				<Col className="d-flex mt-5 flex-row flex-wrap cursor-pointer justify-content-between align-content-center">
						...
						<Container className="d-flex flex-column pt-5">
							...
							<ul className="list-group">
								<li className="list-item">
									<button
										className="btn me-3 btn-link text-white text-decoration-none"
										role="link"
										onClick={() => navigate(`/starships`, { state: { data: "por aqui le pasare los datos de paginacion al padre" } })}>
										<BsArrowLeftShort />
										back
									</button>
								</li>
									...
									<Caja>
										{isLoading ? (
											<>
												...
												Loading external data...please wait
											</>
										) : (
											<></>
										)}
										{dades.map((pilot, index) => (
											<Pilot dadesPilot={pilot} key={index} />
										))}
									</Caja>
								) : null}
                  ...
                  (isLoading)? <><Spinner animation="border" size="sm" className='text-light' />Loading external data...please wait</>:<></>
										...
										{films.map((film, index) => (
											<Films dadesFilm={film} key={index} />
										))}
									</Caja>
								) : null}
							</Display>
